...
async function searchRepos(searchTerm) {
    const reposContainer = document.getElementById('repos');
    const loadingSpinner = document.getElementById('loading-spinner');
    reposContainer.innerHTML = '<h2 class="overview-title">Search Results</h2>'; // Clear previous content and add title
    ...
    try {
        ...
        console.error(`Failed to fetch repositories for search term: ${searchTerm}`, error);
        const errorItem = document.createElement('li');
        errorItem.textContent = `Failed to fetch repositories for search term: ${searchTerm}`;
        reposContainer.appendChild(errorItem);
    }
